    ...
    internal static class ActionIconHooks
    {
        ...
        private static void OnMapChangedHook(ActionMap __instance)
        {
            ...
            if (__instance.no == 22)
            {
                ...
                if (_iconOff == null)
                {
                    _iconOff = (LewdCrestXPlugin.Bundle.LoadAsset<Texture2D>("action_icon_crest_off") ?? throw new Exception("asset not found - action_icon_crest_off")).ToSprite();
                    Object.DontDestroyOnLoad(_iconOff);
                }
                if (_iconOn == null)
                {
                    _iconOn = (LewdCrestXPlugin.Bundle.LoadAsset<Texture2D>("action_icon_crest_on") ?? throw new Exception("asset not found - action_icon_crest_on")).ToSprite();
        ...
        private static bool ShowWindow
        {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    _windowRect = new Rect((_screenRect.width - windowSize) / 2, (_screenRect.height - windowSize) / 2, windowSize, windowSize);

                    _crestableHeroines = Singleton<Game>.Instance.HeroineList.Where(x => x != null && x.isStaff).ToList();
        ...
        private static void CrestWindow(int id)
        {
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal(GUILayout.Height(150), GUILayout.ExpandWidth(true));
                {
                    ...
                    {
                        //todo add toggle for showing crests with no real effect
                        //todo cache these arrays with no crest and make get methods for them to use everywhere
                        _selCrest = GUILayout.SelectionGrid(_selCrest, new[] { "No crest", "blah", "blah2", "blah", "blah2", "blah", "blah2", "blah", "blah2" }, 1, GUILayout.ExpandWidth(true));
                    }
                    GUILayout.EndScrollView();
                    GUILayout.EndVertical();
                    ...
                    {
                        GUILayout.Label("Currently selected crest: " + "Regrow");
                        GUILayout.Label("Effect: " + "Regrow");
                    }
                    GUILayout.EndVertical();
                }
                GUILayout.EndHorizontal();
